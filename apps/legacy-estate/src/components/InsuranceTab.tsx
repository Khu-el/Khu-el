import { Button, Card, NumberInput, TextInput, newId, fromInputValue, toInputValue } from '@nte/governance-core';
...
export function InsuranceTab({ estate, onChange }: { estate: Estate; onChange: (e: Estate) => void }) {
  ...
  const add = () => setRows([...rows, { id: newId('ins'), policyType: '', carrier: '', faceValue: null, cashValue: null, beneficiary: '', annualPremium: null, notes: '' }]);
      ...
      <div className="space-y-3">
        {rows.map((r) => (
          <div key={r.id} className="border border-neutral-200 rounded-lg p-3 grid md:grid-cols-6 gap-2 items-end">
            ...
            <NumberInput placeholder="Face value" value={toInputValue(r.faceValue)} onChange={(e) => update(r.id, { faceValue: fromInputValue(e.target.value) })} />
            <NumberInput placeholder="Cash value (as of last statement)" value={toInputValue(r.cashValue)} onChange={(e) => update(r.id, { cashValue: fromInputValue(e.target.value) })} />
            ...
            <div className="md:col-span-3">
              <NumberInput placeholder="Annual premium" value={toInputValue(r.annualPremium)} onChange={(e) => update(r.id, { annualPremium: fromInputValue(e.target.value) })} />
            </div>
            <div className="md:col-span-3">

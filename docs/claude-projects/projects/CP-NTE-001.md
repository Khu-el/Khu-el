...
npm run build, npm run bus -- validate, npm run bus -- audit. Tests exist only in
packages/neterverse-kernel. There is no linter. State what you actually ran; do not report
"tests pass" where no runner exists.
...
| Khu-el/Khu-el | packages/neterverse-kernel/src/ | The kernel itself — bus, lanes, leases, risk, audit, validation | `PUBLIC` |
